...
export const Collections = ({ onDownload }) => {
  return (
    <div className="pt-6 pb-16 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 animate-fade-in font-sans">
      ...
      <div className="space-y-6">
        {COLLECTIONS_DATA.map((col, index) => {
          ...
          return (
            <div
              key={col.id}
              className="bg-white rounded-2xl border border-[#E1E6E2] p-5 sm:p-6 shadow-sm space-y-4"
            >
              {/* Collection Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#EDF0ED]">
                ...
                <span className="self-start sm:self-auto px-2.5 py-0.5 rounded-full bg-[#EAF1EC] text-[#5B9C8D] text-[11px] font-semibold">
              ...
              {appsInCollection.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {appsInCollection.map((app) => (
                    <div
                      key={app.id}
                      className="p-3 rounded-xl bg-[#F7F8F5] border border-[#E1E6E2] hover:border-[#5B9C8D]/40 transition-all duration-200 flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <img
                          src={app.icon}
                          alt={app.name}
                          className="w-9 h-9 rounded-lg object-cover border border-[#E1E6E2] shadow-sm shrink-0 bg-white"
                        />
                        <div className="min-w-0">
                          <Link to={`/app/${app.id}`}>
                            <h4 className="text-xs font-bold text-[#202522] group-hover:text-[#5B9C8D] transition-colors font-sans truncate">
                              {app.name}
                            </h4>
                          </Link>
                          <p className="text-[11px] text-[#6F7772] truncate">
                            {app.tagline}
                          </p>
                          <span className="text-[10px] text-[#9AA19C] font-mono">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 ml-2">
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-white hover:bg-[#EAF1EC] text-[#5B9C8D] border border-[#E1E6E2] transition-colors"
                          title="Download APK"
                        >
                        ...
                        <Link
                          to={`/app/${app.id}`}
                          className="p-1.5 rounded-lg bg-[#202522] hover:bg-[#343d37] text-white transition-colors"
