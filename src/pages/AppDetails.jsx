...
export const AppDetails = ({ onDownload }) => {
  ...
  const handleCopySha = () => {
    if (app.sha256) {
      ...
      addToast('SHA-256 Checksum copied to clipboard', 'info');
      setTimeout(() => setCopiedChecksum(false), 2500);
    }
  };

  return (
    <div className="pt-3 sm:pt-4 pb-16 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8 animate-fade-in font-sans">
      
      {/* 1. Back Navigation */}
      <div>
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#6F7772] hover:text-[#202522] transition-colors group py-1"
        >
      ...
      {/* 2. Case-Study Header */}
      <div className="bg-white rounded-2xl border border-[#E1E6E2] p-4 sm:p-7 shadow-sm space-y-4 sm:space-y-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-[#EDF0ED]">
          ...
          <div className="flex items-start gap-3 sm:gap-4">
            <img
              src={app.icon}
              alt={app.name}
              className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl object-cover border border-[#E1E6E2] shadow-sm shrink-0 bg-white"
            />
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 shrink-0 pt-1 md:pt-0">
            {app.apkUrl && (
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 sm:py-2 rounded-xl bg-[#5B9C8D] text-white text-xs font-semibold hover:bg-[#4e897b] active:bg-[#43756a] shadow-sm transition-all"
              >
            ...
            {app.githubUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2.5 sm:py-2 rounded-xl bg-white border border-[#E1E6E2] text-[#202522] text-xs font-semibold hover:bg-[#EAF1EC] active:bg-[#EDF0ED] transition-colors shadow-sm"
              >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
          ...
          <div className="p-2.5 rounded-xl bg-[#F7F8F5] border border-[#EDF0ED] space-y-0.5">
            ...
            <span className="font-semibold text-[#202522] block font-mono">{app.size || 'Universal'}</span>
            {app.sizeBytes && (
          ...
          <div className="p-2.5 rounded-xl bg-[#F7F8F5] border border-[#EDF0ED] space-y-0.5">
            ...
            <span className="font-semibold text-[#202522] truncate block">{app.targetAndroid || 'Android 14+'}</span>
          </div>
          <div className="p-2.5 rounded-xl bg-[#F7F8F5] border border-[#EDF0ED] space-y-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5 sm:gap-6 items-start">
        ...
        <div className="md:col-span-8 space-y-5 sm:space-y-6">
          ...
          {/* Overview */}
          <div className="bg-white rounded-2xl border border-[#E1E6E2] p-4 sm:p-6 shadow-sm space-y-3">
            <h2 className="text-base font-bold text-[#202522] font-sans">
              About the project
            </h2>
          ...
          {app.sha256 && (
            <div className="bg-white rounded-xl border border-[#E1E6E2] p-4 shadow-sm space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-[#5B9C8D] font-semibold hover:underline flex items-center gap-1 py-0.5"
                >
                  {copiedChecksum ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                  <span>{copiedChecksum ? 'Copied' : 'Copy hash'}</span>
                </button>
              </div>
              <p className="font-mono text-[10px] text-[#202522] bg-[#F7F8F5] p-2.5 rounded-lg break-all border border-[#EDF0ED] select-all">
                {app.sha256}
              </p>
            </div>
          )}
        ...
        <div className="md:col-span-4 space-y-4">
          ...
          {/* Tech Stack */}
          <div className="bg-white rounded-2xl border border-[#E1E6E2] p-4 sm:p-5 shadow-sm space-y-3">
          ...
          {/* Developer Card */}
          <div className="bg-white rounded-2xl border border-[#E1E6E2] p-4 sm:p-5 shadow-sm space-y-2.5">
